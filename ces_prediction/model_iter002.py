# EXPERIMENT: Target-aware fusion. Replace the single shared 2-output fusion head with
# (a) a dedicated recurrent (GRU) CES-history encoder and (b) two per-target heads.
# The TI head sees the fast-diagnostic features (BES/ECEI/MC) + history + time; the VT
# head sees the dedicated history embedding + time ONLY. This is the one controlled
# change this iteration, motivated directly by the input-modality ablation: V_rot skill
# comes almost entirely from CES history (fast diagnostics carry ~no V_rot info and may
# marginally hurt), while T_i genuinely benefits from the fast diagnostics. A shared head
# forces both targets to read identical features; routing per target lets V_rot lean on a
# stronger sequential history pathway while T_i keeps the fast inputs. Capacity is kept
# modest to avoid the known instability from aggressive width/depth scaling.
...
class HistoryEncoder(nn.Module):
    ...
    folded only into the per-sensor CNNs. The final hidden state summarizes the observed
    history up to (but excluding) the fully-masked target timestep.
    """

    def __init__(self, history_channels, time_channels=4, hidden_dim=64, output_dim=64):
        super().__init__()
        self.gru = nn.GRU(
...
class MultimodalCESPredictor(nn.Module):
    ...
    def forward(self, bes, ecei, mc, time_features=None, ces_history=None):
        ...
        hist_feat = self.history_extractor(ces_history, time_features)

        ti_in = torch.cat((bes_feat, ecei_feat, mc_feat, time_feat, hist_feat), dim=1)
        vt_in = torch.cat((hist_feat, time_feat), dim=1)
