# Work Plan (FINAL — RALPLAN-DR short mode): High-Variability (Peak) CES Reconstruction Evaluation, Examples & Cause Analysis

- Plan ID: ces-peak-recon-plan
- Source spec: `.omc/specs/deep-interview-ces-peak-recon-eval.md` (PASSED, ambiguity 19%)
- Date: 2026-06-23
- Status: **pending approval**
- Consensus: Critic APPROVED v3; this FINAL folds in 4 non-blocking nits (see CHANGELOG).
...
- **Loop cost (NIT 2):** AC8's in-loop work is the CHEAP headline peak skill + bootstrap CI (reusing the npz `run_comparison` already wrote — NO extra forward pass). The AC7 `no_fast` ablation (one extra eval-forward) runs BEHIND a flag (`CES_PEAK_ABLATION=1`), on-demand / outside the AutoML loop.
...
- **Researcher read path (critical for AC8):** the loop's `build_briefing` (`automl_agent_loop.py:413-443`) reads the **IN-MEMORY** `result.get("eval")` captured at `automl_agent_loop.py:326` (`"eval": eval_report`), where `eval_report` is the in-memory return of `run_clean_eval` (`automl_agent_loop.py:311`); it NEVER re-reads `eval_metrics.json` from disk (`:414`). Critically, **`run_evaluation` only calls `run_clean_eval` + `run_comparison`** (`automl_agent_loop.py:309-313`) and assembles `result` at `:324-332` — it **never invokes `peak_analysis.py`**. Therefore a peak block merged onto the on-disk `eval_metrics.json` would NOT reach the briefing. `comparison_metrics.json` reaches ONLY `comparison_skill` scoring (`automl_agent_loop.py:117-133,290,318`), NOT the researcher prompt. The `program` text is injected verbatim into the prompt at `automl_agent_loop.py:475`. **So AC8 requires an additive in-loop edit (Option 1): compute the CHEAP peak block inside `run_evaluation` and merge it into `eval_report` IN MEMORY before `result` is assembled, plus surface it in `build_briefing`, plus the standing `program.md` rule.**
- **`run_comparison` split-tag coupling (NIT 4):** `run_comparison` sets `CES_SPLIT_TAG="val"` on a COPY of env (`compare_env = dict(env); compare_env["CES_SPLIT_TAG"]="val"`, `automl_agent_loop.py:287-288`), runs `compare_baselines.py`, and reads from `self._output_dir(env)` (`:290`). So the npz it writes is `comparison_errors_val.npz` in the shared output dir. `run_peak_eval` must read THAT same val-split npz (match the split tag).
- `[project.optional-dependencies]` header is `pyproject.toml:17`; `dev` extra at `pyproject.toml:18-20`. Insert a `viz` extra adjacent.
...
## Acceptance Criteria (restated, each testable)
...
- **AC5 — example selection (exact, deterministic, overlap-guarded).** Best/median/worst by per-sample skill (improvement over interpolation), k per band (default k=3, configurable 3–5), exact index math + stable tie-break, with a small-pool guard so bands never silently overlap.
  - *Testable:* unit test: for fixed per-sample skills, selection returns the exact expected indices; deterministic; median band uses the specified index math (Step 5); when `n_peak < 3*k` the effective k is reduced (or fewer examples emitted) and a `bands_overlap_guard` note is recorded — verified by a test with a tiny pool.
- **AC6 — visualization.** Per selected example, a CES time-series figure overlaying observed truth / model prediction / PCHIP (and linear) with the peak point highlighted, saved via matplotlib `savefig`. Figures generated for the headline set (i-a) at minimum; (i-b)/(ii) figures optional.
  - *Testable:* the figure step produces N = peak_sets×bands×targets×k_eff `.png` files in `CES_OUTPUT_DIR/peak_figures/` (peak_sets ≥ 1, the headline i-a); `Agg` backend; no `plt.show`; `--no-figures` skips it; import guarded so non-viz paths never require matplotlib.
- **AC7 — cause-analysis ablation (flag-gated own forward pass, paired-shot delta, OOD-disclosed).** `no_fast` (zero BES/ECEI/MC) peak-subset skill reported alongside `full`; a significant drop demonstrates multimodal usage. Ablated arm reuses the IDENTICAL keep+peak subset; paired-shot delta with pinned sign convention. **Runs behind `CES_PEAK_ABLATION=1` (analysis deliverable), NOT every AutoML iteration.**
  - *Testable:* with `CES_PEAK_ABLATION=1`, JSON has `peak.input_only.full` vs `peak.input_only.no_fast` per target with each arm's skill + the paired-diff CI. Significance of "removing fast diagnostics hurts (full is better)" is judged by **`paired_diff_ci95[0] > 0`** (the diff LOWER bound > 0), where `paired_diff = SE_no_fast − SE_full` (positive ⇒ full better). **`_bootstrap`'s own `pass`/`skill` fields MUST be disregarded for the ablation arm** (with this argument order they test the opposite direction — see Step 4). RISKS discloses the OOD-zeros confound. Without the flag, the ablation block is absent and the cheap headline peak metrics are still produced.
- **AC8 — loop integration (reaches the researcher IN-LOOP, Option 1: real numbers, CHEAP only).** An additive edit in `run_evaluation` (`automl_agent_loop.py`) computes the **cheap headline peak block** (peak skill + bootstrap CI by reusing the val-split `comparison_errors_*.npz` — NO extra forward pass) and merges it into `eval_report["per_target"][name]["peak"]` IN MEMORY before `result` is assembled (`:324-332`); `build_briefing` surfaces per-target peak skill/CI from `result["eval"][...]["peak"]` into the briefing STRING; `program.md` carries the standing steering rule. Default scoring (`comparison_skill`) and keep/discard gate numerically unchanged — peak metrics inform the RESEARCHER (model.py rewrite) only, NEVER the gate. The AC7 ablation is NOT part of this in-loop path.
...
### Must Have
- New module `ces_prediction/peak_analysis.py` (detection both families, peak-subset metric assembly + bootstrap CI, example selection, figures, flag-gated AC7 ablation forward pass) — **reusing** `baselines_interpolation`, `bootstrap_compare._bootstrap`, `evaluate.build_clean_val_subset`, `evaluate._persistence_from_history`, `evaluate.apply_ablation`.
- **Minimal hook in `compare_baselines.py`:** add exactly ONE new npz key per target, `boot[f"{name}_is_peak"] = is_peak[valid]` (input-only sub-signal i-a, the headline), aligned to the per-target `valid` mask (`compare_baselines.py:168-171`), placed alongside the existing boot writes (`compare_baselines.py:217-222`). This makes the **non-ablated peak SEs the byte-identical headline SEs**. The standalone module computes figures/diagnostics/the secondary family/the AC7 ablated arm but is NOT the source of the headline significance numbers.
- **Additive in-loop edit to `automl_agent_loop.py:run_evaluation`** (NOT the data/model contract, NOT training logic, NOT AC1-critical): after `run_comparison` (`:313`), call a thin `run_peak_eval(env)` wrapper (cheap headline peak only; reuses the val-split npz, no extra forward pass) and merge the peak block into `eval_report["per_target"][name]["peak"]` IN MEMORY before `result` is assembled (`:324-332`). Plus ~2 lines in `build_briefing` and the `program.md` paragraph.
...
- NO per-iteration AC7 ablation forward pass in the AutoML loop (flag-gated, on-demand only).
...
### Step 2 — Byte-aligned npz hook for the headline subset (AC1, AC3, AC4)
...
### Step 3 — Peak-subset metric + bootstrap CI, dual-sufficiency guard (AC3, AC4)
- In `peak_analysis.py`, `run_peak_eval(...)`: load `comparison_errors_{split}.npz` (split tag MUST match what `run_comparison` wrote — see NIT 4 / Step 7), mask each target's SE/shot arrays by `{name}_is_peak`, compute per-target `peak_rmse_model`, `peak_rmse_pchip`, `peak_skill_vs_pchip = 1 - MSE_model/MSE_pchip` (and vs linear) in physical units (mirrors `compare_baselines.py:174-191`).
...
- This cheap headline metric (i-a) is what `run_peak_eval` returns for the in-loop AC8 merge. Sub-signal (i-b) and Family (ii) via `detect_peak_rows_target` are computed in-module for the full `peak_metrics.json` report (labeled separately / secondary with the caveat), NOT in the cheap in-loop path.

### Step 4 — AC7 ablation: flag-gated own forward pass, sample-for-sample paired delta (AC7)
- **Flag-gated (NIT 2):** this whole step runs only when `CES_PEAK_ABLATION=1`. It is an analysis deliverable (one extra eval-forward), NOT part of the per-iteration AutoML loop. Without the flag, `peak_metrics.json` omits the ablation block and the cheap headline metrics are still produced.
...
- **Small-pool overlap guard (NIT 1):** let `n_peak = len(skill)`. If `n_peak < 3*k`, reduce the effective `k_eff = max(1, n_peak // 3)` (or emit fewer examples) and record a `bands_overlap_guard: {n_peak, k_requested, k_effective}` note in `peak_metrics.json` so worst/median/best bands never silently overlap or draw from a tiny pool. **Example selection may run on a pool BELOW `N_MIN_PEAK_ROWS`** — the sufficiency guard (Step 3) gates the PASS *verdict*, not example selection; examples are illustrative and are still produced (with the guard note) even when the CI is `insufficient_*`.
- Sort ascending by the composite key `(skill, shot_id, row_index)` (stable tie-break). Let `n = n_peak`, `order` = argsort with that key, `k = k_eff`.
  - `worst  = order[0:k]`
  - `best   = order[n-k:n]`
...
### Step 6 — Figures (AC6)
- Add `matplotlib>=3.7` to a NEW `[viz]` optional-dependencies group in `pyproject.toml`, inserted adjacent to `dev` (header `[project.optional-dependencies]` at `pyproject.toml:17`, existing `dev` at `:18-20`). Never core.
...
### Step 7 — Loop / researcher exposure, IN-LOOP Option 1 (AC8)
> An on-disk `eval_metrics.json` write-back does NOT work: the loop never invokes `peak_analysis.py` (`run_evaluation` only runs `run_clean_eval`+`run_comparison`, `automl_agent_loop.py:309-313`), and `build_briefing` reads the IN-MEMORY `result["eval"]` captured at `:326`, never re-reading the file (`:414`). Use the real in-memory path.
- (a) **`run_peak_eval(env)` wrapper** in `peak_analysis.py` (importable): returns the per-target **cheap headline** peak block dict (sub-signal i-a skill + `peak_skill_ci95` + sufficiency flags). It reuses the val-split `comparison_errors_*.npz` that `run_comparison` just produced — **NO extra forward pass** (NIT 2). It does NOT run the AC7 ablation in-loop. **Split-tag matching (NIT 4):** `run_comparison` set `CES_SPLIT_TAG="val"` on a COPY of env (`automl_agent_loop.py:287-288`) and wrote `comparison_errors_val.npz` in `self._output_dir(env)`; `run_peak_eval` must read that exact `val` npz (do NOT assume the shared `env`'s tag). The Step-4 `np.array_equal` pairing assert (when the ablation runs) will fail loudly on any split/index mismatch — keep it.
...
- (c) **~2 lines in `build_briefing`** (inside the `for name in TARGET_NAMES` loop, `automl_agent_loop.py:422-429`): when `stats.get("peak")` is present, append a line with per-target peak `skill_vs_pchip` + `peak_skill_ci95` to the briefing STRING.
...
### Step 8 — Tests, sweep & docs (AC1, AC9)
- New `tests/test_peak_analysis.py` (CPU-only, synthetic numpy; mirror `test_baselines_interpolation.py` style):
  ...
  - example selection exact-index test (Step 5) **plus the small-pool overlap guard** (`n_peak < 3*k` → reduced `k_eff`, no overlap, guard note recorded);
...
- **(i) Peak subset too small for a valid CI.** → Dual-sufficiency guard: `N_MIN_PEAK_SHOTS=15` binding + `N_MIN_PEAK_ROWS=200`; emit `insufficient_*`, suppress PASS, report point estimate only. Example selection still runs (with the small-pool overlap guard) since it is illustrative, not a verdict.
...
- **(iii) AC8 metric never reaches the researcher.** → An on-disk `eval_metrics.json` write-back does NOT reach the in-memory briefing (loop never calls `peak_analysis`; `build_briefing` reads `result["eval"]` at `:326`, not disk). Fix = additive in-loop edit to `run_evaluation` merging the CHEAP peak block into `eval_report` IN MEMORY before `:324-332`, plus a `build_briefing` line, plus the `program.md` standing rule. Verified by a test that drives `run_evaluation` (stubbed) → asserts `result["eval"][...]["peak"]` populated AND `build_briefing(result)` string contains the peak fields — NOT a synthetic-only test.
...
- **(vi) `is_peak` misaligned to the SE arrays (incl. split-tag mismatch).** → Slice by `valid` (NOT `keep`), aligned to the existing boot writes; read the SAME val-split npz `run_comparison` wrote (NIT 4); assert equal lengths + `np.array_equal` against npz arrays in code + test.
- **(vii) OOD-zeros confound in AC7.** → Disclosed in RISKS; report the drop honestly; a single ablation cannot fully separate "uses fast signal" from "fed OOD zeros". Ablation is flag-gated so it never silently inflates loop cost.
...
- **Loop cost.** AC8 in-loop work reuses the existing npz (no extra forward pass); the AC7 ablation forward pass is flag-gated (`CES_PEAK_ABLATION=1`), so per-iteration loop cost is unchanged. (NIT 2.)
- **Split-tag mismatch.** `run_peak_eval` reads the SAME val-split npz `run_comparison` wrote (`CES_SPLIT_TAG="val"` on a copy of env, `automl_agent_loop.py:287-288`); `np.array_equal` pairing assert catches any mismatch. (NIT 4.)
...
- **Example-band overlap on a tiny pool.** Small-pool guard reduces `k_eff` and records a note. (NIT 1.)
...
3. With real data: `python ces_prediction/peak_analysis.py` → `CES_OUTPUT_DIR/peak_metrics.json` (AC3/AC4/AC5) + `peak_figures/*.png` (AC6). With `CES_PEAK_ABLATION=1`, the AC7 ablation block is added. AC8's in-memory merge is exercised separately via the `run_evaluation` test (step 5), since the standalone CLI run does not go through the loop.
4. Inspect `peak_metrics.json`: headline `peak.input_only.ces_activity` `peak_skill_vs_pchip`, `peak_skill_ci95`, `pass`/`insufficient_shots`; separate `peak.input_only.fast_activity`; Family (ii) `peak.observed_target` with caveat; (with the flag) AC7 `paired_diff_ci95` (judged by lower bound > 0); `examples` best/median/worst + `bands_overlap_guard` note.
...
6. `peak_figures/*.png` count == peak_sets×bands×targets×k_eff (headline i-a at minimum); visually confirm overlay + peak highlight (AC6).
...
3. **AC8 actually reaching the researcher (cheaply)** — peak metrics merged into the IN-MEMORY `eval_report` inside `run_evaluation` (reusing the existing npz, no extra forward pass) + surfaced in the `build_briefing` string + a `program.md` standing rule; an on-disk-only `eval_metrics.json` write would never reach the briefing, and a per-iteration ablation would inflate loop cost.
...
- **Option A + minimal Option-B npz hook + in-loop cheap AC8 merge (CHOSEN).** Standalone module for detection/figures/sub-signal i-b/secondary family/flag-gated ablation; ONE additive npz key (`{name}_is_peak`, sliced by `valid`) in `compare_baselines.py` so headline peak SEs are byte-identical; AC8 via an additive in-memory merge of the CHEAP headline peak block in `run_evaluation` + `build_briefing` line + `program.md`. Pros: byte-aligned headline, minimal AC1 surface, AC8 reaches the researcher in-loop with REAL numbers at no extra forward-pass cost. Cons: peak metrics produced in two places (npz headline SEs + module report), and the AC8 path adds a small `automl_agent_loop.py` edit (additive, gate-neutral).
...
- **Decision:** Implement peak reconstruction evaluation as a standalone `ces_prediction/peak_analysis.py` PLUS one additive `{name}_is_peak` npz key (sliced by `valid`) in `compare_baselines.py`. Headline = Family (i) input-only sub-signal (i-a) CES-activity = high-local-activity NEIGHBORHOODS (target-independent proxy, not pointwise extrema); sub-signal (i-b) fast-diagnostic activity reported separately; secondary = Family (ii) observed-target peaks (caveated). The AC7 `no_fast` ablation runs its own forward pass in the module, paired sample-for-sample to the npz `{name}_se_model`/`{name}_shot`/`{name}_is_peak`, judged ONLY by `paired_diff_ci95[0] > 0` (`_bootstrap`'s `pass`/`skill` disregarded), and is FLAG-GATED (`CES_PEAK_ABLATION=1`), NOT run per AutoML iteration. AC8 exposure is IN-LOOP (Option 1): additive edit to `automl_agent_loop.py:run_evaluation` merges the CHEAP headline peak block (reusing the val-split npz, no extra forward pass) into the in-memory `eval_report` before `result` assembly (`:324-332`) + `build_briefing` line + `program.md` standing rule. matplotlib as `[viz]` optional extra, figures skippable.
- **Drivers:** non-circular defensible headline; AC1 numeric non-regression; AC8 reaching the researcher's actual read path cheaply; shot-bound CI validity.
- **Alternatives considered:** standalone-only (drift in headline SEs); heavy `compare_baselines.py` extension (AC1 blast radius); notebook/ad-hoc (non-reproducible); target-defined peaks as headline (circular); per-iteration in-loop ablation (loop-cost inflation — rejected via flag-gating).
- **Why chosen:** byte-aligned headline SEs with minimal AC1 surface; the input-only peak family rebuts the circularity objection; the in-loop in-memory merge of the cheap peak block into `eval_report` + `build_briefing` is the only path that actually reaches the researcher prompt (an on-disk `eval_metrics.json` write is never re-read, and `comparison_metrics.json` only feeds the gate); flag-gating the ablation keeps per-iteration cost flat.
- **Consequences:** peak metrics split across `comparison_errors_*.npz` (headline SEs) + `peak_metrics.json` (full per-family report + figures + flag-gated ablation) + an in-memory `eval_report[...]["peak"]` block merged inside `run_evaluation`; additive edits to FOUR files — `compare_baselines.py` (one npz key), `automl_agent_loop.py` (`run_evaluation` merge + `build_briefing` line), `program.md` (standing rule), `pyproject.toml` (`[viz]`). The `run_evaluation` edit is additive (try/except-guarded), touches neither the data/model contract nor training logic nor the keep/discard gate. matplotlib only needed for figures.
- **Follow-ups:** if input-defined peak skill is weak / CI straddles 0 (esp. CES_VT), the `program.md` steering lets the AutoML loop auto-propose a peak-weighted loss (the deferred non-goal); revisit `N_MIN_PEAK_SHOTS` if the peak subset spans many shots; consider a manifold-respecting ablation (noise/permutation instead of zeros) to address the OOD confound; consider promoting the flag-gated ablation into the loop only if loop budget allows.

---

## Open Questions for user (non-blocking)
- **(a) Loop-cost tolerance** — RESOLVED by NIT 2's design (cheap headline peak metric in-loop with no extra forward pass; AC7 ablation flag-gated and out-of-loop). Confirm this matches your loop-budget expectations; if you want the ablation in every iteration, say so and we lift the flag.
- **(b) "the loop auto-recommends" intent** — the spec (AC8 / Topology deferred row) says peak weakness should let the loop auto-recommend peak-weighted training. This plan satisfies that via TWO channels: live per-iteration peak skill/CI in the briefing STRING + a standing steering rule in `program.md`. Confirm this combined approach is the intended reading of "auto-recommends" (the researcher proposes the change; the loop does not silently flip the loss). v3's combined approach is a reasonable reading; flag if you expected a stronger/automatic mechanism.

---

## CHANGELOG (v1 → v2 → v3 → final)
- **v1 (initial draft).** RALPLAN-DR with Requirements/AC1-AC9/Steps/Risks/Verification; proposed standalone `peak_analysis.py` + npz hook; left 4 open questions (thresholds, AC8 exposure, N_MIN, matplotlib).
- **v2 (after Architect/Critic REJECT of v1).** Resolved the 4 open questions as decisions. Added the **two-family** peak definition (input-only headline vs observed-target secondary) to kill the **circularity** (G2). Made **AC1 a numeric non-regression gate** (G5). Added the **dual-sufficiency (shot-bound) guard** with `N_MIN_PEAK_SHOTS` (G1/AC4), correcting the `>50` ROW vs SHOT confusion. Pinned the **`is_peak[valid]`** slice (G3/G6). Gave AC7 its **own forward pass** with a paired-shot delta (G4). Added **pre-mortem (7)** and the **ADR**. Routed AC8 to `eval_metrics.json` + briefing + `program.md`.
- **v3 (after Architect re-review of v2: SOUND-WITH-CHANGES).** **(1, BLOCKING/AC8)** Replaced the dead on-disk `eval_metrics.json` write-back with an **in-loop in-memory merge** in `run_evaluation` (the loop never invokes `peak_analysis` and `build_briefing` reads in-memory `result["eval"]` at `:326`); AC8 test now drives the REAL path. **(2/AC7)** Pinned the ablation **sign convention** to `paired_diff_ci95[0] > 0` and to DISREGARD `_bootstrap`'s `pass`/`skill` for the ablation arm. **(3/Family i)** Reworded the headline to **high-local-activity NEIGHBORHOODS (proxy, not pointwise extrema)** and split out **(i-a) CES-activity** (headline) vs **(i-b) fast-diagnostic activity** (separate). **(4/pairing)** Require aligning the ablation arm to the EXACT npz indices via `np.array_equal` (fail loudly on mismatch).
- **final (after Critic APPROVE of v3; 4 non-blocking nits folded).**
  - **NIT 1 (AC5 overlap):** added the small-pool guard — if `n_peak < 3*k`, reduce `k_eff` (or emit fewer) and record `bands_overlap_guard`; clarified example selection may run below `N_MIN_PEAK_ROWS` (the sufficiency guard gates the PASS verdict, not example selection).
  - **NIT 2 (loop cost):** split AC8's CHEAP in-loop headline peak skill+CI (reuses the npz, no extra forward pass) from the AC7 ablation forward pass, which is now FLAG-GATED (`CES_PEAK_ABLATION=1`), on-demand / out-of-loop. Updated AC7/AC8, Steps 3/4/7, RISKS, pre-mortem, ADR.
  - **NIT 3 (pyproject cite):** corrected to `[project.optional-dependencies]` header `pyproject.toml:17`, `dev` at `:18-20`; insert `viz` adjacent (Step 6).
  - **NIT 4 (split coupling):** `run_peak_eval` must read the SAME val-split npz `run_comparison` wrote (`CES_SPLIT_TAG="val"` on a copy of env, `automl_agent_loop.py:287-288`); kept the `np.array_equal` pairing assert as the loud failure guard (Step 7 / RISKS / pre-mortem vi).
  - Added an **Open Questions for user** section ((a) loop-cost tolerance resolved by NIT 2; (b) whether briefing numbers + `program.md` steering fully satisfy the spec's "loop auto-recommends" intent). Status set to **pending approval**; this `.omc/plans/` copy is authoritative.
