...
1 & 재구성 대상이 \emph{희소 이온} 채널이다(CES \Ti{}·\Vrot{})
...
2 & 타겟 자신의 \emph{불규칙한 과거}에 조건화된 인과 추정이고, 미래를 절대 읽지 않는다
...
3 & 이겨야 할 기준선을 \emph{타겟 채널의 미래 사용 보간}으로 세우고 그것을 착수 전에 등록한다
...
4 & 재구성 가능성을 \emph{가정하지 않고 타겟별로 검정}한다
...
5 & 계측기 유지값을 채점에서 제거하고, 스펙트럼 피팅 실패를 \emph{두 공동 1차 모집단}으로 괄호 친다
...
5 & \textbf{값 기준 컷은 대리 지표다.} 두 공동 1차 모집단이 피팅 실패 문제를
  ...
  & CES 피팅 품질 메타데이터($\chi^2$·신호세기)를 받아 값 기준 컷을 품질 기준 컷으로
    바꾸는 것(\S\ref{sec:headroom}). 이것이 이 문제를 결말짓는다 \\
