...
FUSION_ROWS = [
    ...
    ["이분산 신경망 대리모델 (arXiv 2608.19377)", "Thomson 산란", "2026",
     "잡음이 지배하는 분광 측정에서 MCMC 파라미터 추정을 가속한다",
     "이중 헤드 신경망이 기대 스펙트럼과 채널별 잡음 분산을 함께 내고 가우시안 NLL로 학습한다",
     "정확 물리 모델 대비 1,500배 이상 가속",
     "우리 열린 축 1번(이분산 손실)의 핵융합 도메인 선례. 등분산 기준선 대비 RMSE를 20 % 이상 "
     "줄였다 — 손실 형태를 바꾸는 것이 이 분야에서 실제로 값을 한다는 증거"],
    ["XICS ↔ CES 교차 검증", "KSTAR", "2009 ~",
     "He 유사 아르곤 도플러로 Ti·토로이달 회전을 독립 측정한다",
     "X선 결정 분광기(XICS). 최근 LabVIEW 실시간 처리로 준실시간화",
     "코어 Ti·회전이 여러 방전에서 CES와 잘 일치",
     "KSTAR에는 회전을 재는 두 번째 진단이 이미 있다. 우리 641 shot에 XICS가 있다면 V_rot 분기의 "
     "추가 입력이자 독립 라벨원이 된다 — B.6와 나란히 놓을 새 레버"],
    ["TorbeamNN (arXiv 2504.11648)", "KSTAR", "2025",
     "ECH 거울 조준을 위한 TORBEAM 광선추적 대리모델",
     "신경망 대리모델이 실시간 제어계(PCS)에서 동작",
     "실시간 구현 대비 100배 이상 가속, 흡수 위치 평균 오차 0.5 cm, 실험 중 검증",
     "KSTAR PCS에 신경망이 이미 실장되어 돌고 있다. 우리 배치 주장(§8ah·§8aj)이 가정이 아니라 "
     "이 장치의 기존 관행임을 보여준다"],
]
...
GENERAL_ROWS = [
    ...
    ["이분산 · 강건 회귀", "2023 ~ 26",
     "가우시안 NLL 병리, β-NLL, Toto의 Student-t, 이분산 신경망 대리모델(Thomson, 2026)",
     "예측 분산이 평균 그래디언트를 오염시키므로 stop-grad나 별도 헤드 필요. 핵융합 분광에서는 "
     "이중 헤드 이분산이 등분산 대비 RMSE 20 % 이상 개선",
     "§8aq가 지목한 유일한 미개봉 축이고 이제 핵융합 도메인 선례도 있다. conformal은 사후 구간이지 "
     "학습 손실이 아님"],
    ["표형 모델", "2025",
...
PRIORITY_ROWS = [
    ...
    ["5", "V_rot 입력 확장: 원시 kHz Mirnov 특징(B.6 A1) · NBI 토크 채널 · KSTAR XICS 회전 채널",
     "EAST XCS · XICS↔CES 교차 검증 · PanoMHD", "데이터 획득",
     "변동 3분위 V_rot 승률이 먼저 오를 것"],
...
SURVEY_SOURCES = [
    ...
    ("이분산 신경망 대리모델 (Thomson)", "https://arxiv.org/abs/2608.19377"),
    ("KSTAR XICS 실시간 Ti 측정", "https://doi.org/10.1016/j.fusengdes.2023.113570"),
    ("TorbeamNN (KSTAR PCS 실장)", "https://arxiv.org/abs/2504.11648"),
    ("KSTAR UFO 사건 예측 (NF 2026)", "https://doi.org/10.1088/1741-4326/ae91fc"),
