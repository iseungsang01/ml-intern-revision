...
> 인용 53에 **번호 붙은 수식이 0개**이다. 1시간 덱이 56 → **57장**이 되었다 — 4장(모델)에 **"물리적 근거: 두 갈래
> 라우팅은 설계 선택이 아니라 두 보존식의 결과이다"**(19번 슬라이드)를 신설하여 에너지·각운동량 보존식을 적고,
> **각운동량 수지를 항별로 감사**한 표(좌변 측정 · T_NBI 부재 · T_NTV 음성 종결 · ∇·Π_turb 이번 측정 널 ·
> T_intrinsic 도달 불가)와 측정된 결합(BES·ECEI → Tᵢ 값 +0.341/+0.311 · 변화율 +0.070/+0.078 vs V_rot
> +0.027/+0.005 · -0.006/-0.003), 시간 척도 일치(τ_eq 8~59 ms ↔ 문맥 포화 약 50 ms)를 실었다. 종합방어 덱의
> 예상질문 ③ 첫 행(V_rot는 왜 동률인가)도 같은 항별 감사로 다시 썼다. 근거는 `THESIS_RESULTS.md` §8ar,
> 산출물은 `ces_prediction/analyze_physics_scales.py` → `data/.physics_scales.json`. 슬라이드에 새로 쓰는 기호는
> 두 폰트에 모두 없는 ⟨⟩·⋯·U+2212를 피해 ASCII로 적었다. FIT WARNING 0.

> **✅ 2026-09-03 20분 덱 압축 (승상님 지시).** 발표자 노트의 시간 배정과 화면 글자 수를 대조하니 배정된 시간 안에
> 물리적으로 말할 수 없는 분량이었다(슬라이드 4는 35초에 812자, 12는 45초에 1,021자, 23은 55초에 1,116자;
> 전체 분당 790자로 한국어 청취-병행 독해 속도의 약 2배). 문체(초록체)와 24장 구성은 그대로 두고 **화면 글자만**
> 줄였다: 자체 슬라이드 6장(2·3·7·12·16·23)은 카드 줄 수를 줄여 다시 썼고, 1시간 덱에서 재사용하는 7장
> (4·5·8·11·18·22)은 `drop_para`/`set_para`로 덜어냈다(1시간 덱은 영향 없음). 내려간 사실은 전부 발표자 노트로
> 옮겼다. 이어서 **러닝 클록을 재배분**하였다 — 그림 슬라이드(9·14·17·21)에서 남던 초를 글자 밀도가 높은
> 슬라이드로 옮겼고 총 시간 19:20은 그대로이다. 결과: 화면 글자 **15,284 → 13,027자(−15%)**, 분당 790 → 674자,
> 최악 슬라이드 **21.8 → 13.7 자/초**, 14 자/초를 넘는 슬라이드 0장. 빈 공간이 생긴 2·12·23번은 상자 높이와
> 본문 크기를 다시 잡았고, 18번 제목은 §8aq에 맞추어 "총합 손실의 상한"으로 한정하였다(20분 덱에는 재현성 상한
> 슬라이드가 없으므로 제목만으로 과장되지 않게). FIT WARNING 0.

> **✅ 2026-09-03 재현성 상한(§8aq) 반영 + 선행연구 슬라이드 신설.** 1시간 덱이 54 → **56장**이 되었다.
> (1) **선행 연구와의 위치**(1장 끝, 9번 슬라이드) — 가장 가까운 6편(Diag2Diag · COMPASS · RTCAKENN ·
> EAST XCS · FusionMAE · KSTAR EPED)과 각각의 차이를 표로 두고, `docs/paper/NOVELTY.md`의 표현 규칙대로
> 부재가 아닌 **세 축 확장**(타깃 채널 · 인과성 · 평가 기준)으로 마무리한다. 방어 자리에서 반드시 나오는
> 질문인데 1시간·20분 덱 어디에도 없었다. (2) **결과 ⑩ 재현성 상한**(33번 슬라이드) — 타깃 자신의 10 ms
> 재현성을 차분 기반 추정기로 재어(`ces_prediction/analyze_noise_floor.py` → `data/.noise_floor.json`,
> 641파일 전수) 백본의 157.8 eV를 절대 단위와 나란히 놓는다. 이에 따라 **결과 ⑪·⑫·⑬으로 번호가 밀렸다**.
> (3) 결론 ⑤와 종합방어 덱의 판정표·예상질문 ②를 **"총합 MSE에 한정"**으로 한정하였다(꼬리 통계이므로
> 본류에는 2.3~3.4배의 여지가 남는다). (4) 연구흐름 덱 표지의 넘침 1건을 고쳤다. 빌드 로그 FIT WARNING 0
> (네 덱 모두). 근거는 `THESIS_RESULTS.md` §8aq.
...
> 계열·문맥 질문을 추가했다. **종합방어 덱**은 판정표에 B.9 행(문맥 포화·계열 동률·1 ms 보류)을 더하고 B.9 슬라이드 2장과
> 예상 질문 ④(왜 LSTM / 문맥 길이 / 1 ms / V_rot 검정력)를 신설했으며, 결정 기록을 08-16·08-21·08-24의 4건으로 갱신했다.
> **연구흐름 덱**은 `main_ko.tex`(2026-08-16판, B.9 미반영)의 절 구조를 유지한 채 §6.12(추가 예정) 슬라이드 한 장에 B.9 절의
> 골격·인용 수치·쓰지 않는 문장을 정리했고, §9·§10·§11 슬라이드에 그 함의를 반영했다. 빌드 로그 FIT WARNING 0 (네 덱 모두).
...
→ ④ 데이터·문제 설정 §3 → ⑤ 감사 2종 §3.4–3.5 → ⑥ 모델 §4 → ⑦ 평가 방법론 §5 →
⑧–⑱ 결과 11장(§6.1–§6.11) → ⑲ 모델 선택 §7 → ⑳ **§6.12(추가 예정) B.9** → ㉑ 배치 §8 → ㉒ 개선 여지 §9 → ㉓ 한계·결론 §10–11.

- **2026-08-27 판(초록체).** `main_ko.tex`는 2026-08-16 이후 개정되지 않았으므로(B.9 §8ac–§8an은 논문 본문에 아직 없다)
  §3–§11 슬라이드는 논문과 일치한다. 20번 슬라이드가 논문에 추가할 §6.12의 골격(§9 순서: 질문 → 절단 사다리의 세 교정 →
  통합 재채점 → 계열 → 비용 → 승패 방전 → 쓰지 않는 문장)과 인용 수치를 담는다.
