...
| 파일 | 형식 | 용도 |
|---|---|---|
| **`KSTAR_CES_발표자료.pptx`** | PowerPoint, 71 슬라이드 (16:9) | **약 1시간 학위논문 발표용** 덱 (2026-09-05 판 · 초록체 · §8ac–§8ar 반영 · 선행연구/재현성 상한/물리적 근거 슬라이드 + **부록 A 8장**) |
...
| **`KSTAR_CES_졸논정리_논문흐름.pptx`** | PowerPoint, 65 슬라이드 (16:9) | **`docs/졸논 정리.pptx` 형식**(분할 테마 · 구역 표지 · 제목+불릿)을 그대로 물려받아 `main_ko.tex`의 절 순서대로 채운 덱 (2026-09-04) |
| **`KSTAR_CES_1pager.pdf`** | A4 PDF, 1 페이지 | **한 장 요약** (배포용; 2026-08-27 초록체 재작성 · B.9 반영) |
| `KSTAR_CES_1pager.png` | PNG | 1-pager 미리보기 이미지 |

> **⚠️ 2026-09-05 3차 조사 + 페이지 숫자 버그 (승상님: "NN-CES도 핵융합 문헌에 넣어야 할 거 아니야").**
> NN-CES는 2차 조사에서 이미 표에 들어가 있었지만 **탭 숫자가 12로 굳어 있어** 들어가지 않은 것처럼
> 보였다. `build_survey_artifact.py`의 모든 개수(탭 배지 · 리드 · 필터 칩 · 사실 스트립)를 이제
> `appendix_content.py`에서 **계산**한다. 같은 부류의 버그가 하나 더 있었다 — 배지 상태가 행과 나란한
> **위치 기반 리스트**여서 21번째 행이 조용히 잘려 나갔다. 이제 행 이름 접두사로 조회하며, 항목이 없는
> 행은 빌드가 **실패**한다. 3차 조사로 세 편을 더 넣어 핵융합 표는 **21편**이 되었다:
> **이분산 신경망 대리모델**(Thomson, arXiv 2608.19377 — 이중 헤드가 스펙트럼과 채널별 잡음 분산을
> 함께 내고 등분산 대비 RMSE 20 % 이상 개선, 우리 열린 축 1번의 핵융합 선례), **KSTAR XICS**(He 유사
> 아르곤 도플러로 Ti·회전을 재는 **두 번째 진단**이며 CES와 잘 일치 — V_rot 입력 확장 팔에 추가),
> **TorbeamNN**(KSTAR PCS에 실장되어 도는 신경망 대리모델, 100배 가속 — 우리 배치 주장이 이 장치의
> 기존 관행임을 보여준다). 출처 45 → 49건, 시도한 모델 21건, 다음 팔 7행.

> **✅ 2026-09-05 §8at — NN-CES가 부른 추론을 측정으로 철회하였다.** NN-CES의 초록이 밝힌 계측 사실
> (KSTAR는 전용 진단빔이 아니라 **주 가열빔 변조**로 CES를 잰다)에서 "그렇다면 우리 결측은 그 변조
> 주기이고 변조 없는 분석이 결측을 없앤다"는 추론이 곧바로 부록에 적혔다. 그 문장을 측정으로 검정하였다
> (`ces_prediction/analyze_ces_duty_cycle.py` → `data/.ces_duty_cycle.json`, 641파일 전수 · 모델 없음).
> **결과는 아니다** — 두 타깃 마스크의 일치는 **원본 72.8 %**이고 우리 3 keV 컷은 그것을 0.3 %p만
>움직인다(공통 셔터라면 거의 100 %여야 한다). 마스크 자기상관은 잔물결 없이 매끄럽게 감쇠하고
> (Tᵢ 0.327 → 0.123), 우세 주기는 1.48 s · 2.98 s로 듀티 사이클이 아니라 블록 포락선 규모이다.
> Tᵢ 결측은 7,169회 중 4,028회가 **한 프레임(10 ms)짜리 고립 결측**이다. 우리가 받은 행은 이미 배경
> 차감과 적합이 끝난 10 ms 산출물이므로 변조는 상류에서 접혀 들어갔다. 남는 이득은 **프레임별 적합
> 품질**이고, 그것이 값 기준 3 keV 컷을 대체해 두 공동 1차 모집단을 하나로 합친다. 부록의 해당 문장과
> 우선순위 4번 팔을 그렇게 고쳤고, 시도한 모델 표에 §8at 행을 더했다(20 → 21건).
...
py docs/presentation/build_pptx.py                     # 2) 1시간 덱 71장(부록 A 14장 포함) (헬퍼는 다른 덱이 import; 7장은 docs/paper/figures/fig_context_family_ladder.png를 읽음)
...
py docs/presentation/build_pptx_jolnon.py              # 5b) 졸논정리 형식의 논문 흐름 덱 65장 (docs/졸논 정리.pptx를 템플릿으로 연다)
