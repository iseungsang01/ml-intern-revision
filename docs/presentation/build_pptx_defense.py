...
fband(s, 0.55, 6.78, 12.25, 0.28,
      [("†  캠페인 = 한 시간 블록 × 초기화 4개.   ‡  persistence 대비는 두 모집단 4/4.   §  V_rot는 persistence 3/4 · 캠페인 4/4 · Δt > 15 ms PASS(양쪽); 우위는 소수 방전에 집중.   B.9 행은 컷 모집단·통합 301 방전.",
...
ftable(s, 0.55, 1.42, _QA_COLS, _QA_HEAD,
       [
           ...
           ["b3k8 = 백본이라면서 왜 컷 조건부인가",
            ...
            ("●●●", GREEN, True)],
           ["크기를 키우면 되지 않는가",
            "B.4가 그 축을 닫았다. Tᵢ 인코더 폭 24 → 260(34k → 879k, 26배)에서 평균 skill이 +0.230/+0.236/+0.235/+0.236/+0.230으로 160 대비 ±0.008이고, 최대 폭의 유의 우세는 1/4이며 V_rot는 불변(+0.250~+0.254)이다. 반대편에서는 21,498 파라미터 b3k8이 컷 모집단에서 백본과 동급이고, B.9의 1,808 파라미터 tcn2k도 인과 GP를 4/4로 이긴다. 상한은 추정기 용량이 아니라 정보이다.",
            ("●●●", GREEN, True)],
       ],
