...
### What's New: Autonomous ML R&D Workflow
The system has been upgraded from a generic coding assistant to a **Multi-Agent AI Scientist**. It introduces specialized agents (Evaluation, Briefing, Researcher) that autonomously run training loops, track validation loss histories, detect plateaus, and modify model architectures (e.g., from CNNs to Transformers) to continuously improve performance.
